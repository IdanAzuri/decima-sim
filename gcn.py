...
class GraphCNN(object):
    def __init__(self, inputs, input_dim, hid_dims, output_dim,
        ...
        self.prep_calibration_factor =  tf.Variable(1/len(self.prep_weights))#self.init_factor()

        # f: x' -> e
        self.proc_weights, self.proc_bias = \
            self.init(self.output_dim, self.hid_dims, self.output_dim)
        self.proc_calibration_factor =  tf.Variable(1/len(self.proc_weights))
        # g: e -> e
        self.agg_weights, self.agg_bias = \
            self.init(self.output_dim, self.hid_dims, self.output_dim)
        self.agg_calibration_factor = tf.Variable(1/len(self.agg_weights))
